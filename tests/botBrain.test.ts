...
describe('BotBrain 优先级', () => {
  ...
  it('残血压过交战 → retreat', () => {
    const b = new BotBrain(); b.reset(r0);
    b.think(openWorld(), self0(0, 0, 20), [T('p', 0, 8)], noSquad, 0.1, true, r0);
  ...
  it('看不见但有共享情报+被派绕后 → flank', () => {
    ...
    b.think(openWorld(), self0(), [T('p', 0, 40)], squad, 0.1, true, r0); // 玩家在40米外看不见
  ...
  it('看不见但有共享情报+正面 → hunt', () => {
    ...
    b.think(openWorld(), self0(), [T('p', 0, 40)], squad, 0.1, true, r0);
  ...
  it('战斗未开启(准备阶段)时就算玩家在眼前也 patrol', () => {
    const b = new BotBrain(); b.reset(r0);
    b.think(openWorld(), self0(), [T('p', 0, 5)], noSquad, 0.1, false, r0);
    expect(b.drive.mode).toBe('patrol');
    expect(b.drive.weaponsFree).toBe(false);
  });
  it('记忆超8秒遗忘 → 回到 patrol', () => {
    ...
    b.think(openWorld(), self0(), [T('p', 0, 40)], noSquad, 9, true, r0); // 9秒后才想
    expect(b.drive.mode).toBe('patrol');
  });
});
...
describe('BotBrain 掩体交战(躲→探头循环)', () => {
  ...
  it('交战会先跑向掩体，到位后进入躲/探头循环', () => {
    ...
    b.think(w, self0(2.5, -0.7), [T('p', 0, 7)], noSquad, 0.1, true, r0);
    expect(b.drive.mode).toBe('combat');
    expect(b.coverI).toBe(0);
    // 2) 传送到掩体上再想 → 到位,开始"躲"(不开火)
    b.think(w, self0(0, -0.7), [T('p', 0, 7)], noSquad, 0.1, true, r0);
    expect(b.drive.weaponsFree).toBe(false);
    // 3) 躲够了 → 探头(有探头点,开火放行)
    b.think(w, self0(0, -0.7), [T('p', 0, 7)], noSquad, 2, true, r0);
    expect(b.drive.weaponsFree).toBe(true);
  ...
  it('玩家躲起来3秒内仍算交战(粘性),超时转 hunt', () => {
    const b = new BotBrain(); b.reset(r0);
    const w = openWorld([], []);
    b.think(w, self0(), [T('p', 0, 8)], noSquad, 0.1, true, r0);
    expect(b.drive.mode).toBe('combat');
    // 玩家跑到40米外(看不见了),1秒后想:还在combat粘性期
    b.think(w, self0(), [T('p', 0, 40)], noSquad, 1, true, r0);
    expect(b.drive.mode).toBe('combat');
    // 再过3秒:粘性过期 → hunt(去最后位置找)
    b.think(w, self0(), [T('p', 0, 40)], noSquad, 3, true, r0);
    expect(b.drive.mode).toBe('hunt');
  });
});
