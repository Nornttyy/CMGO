...
interface MRect { x0: number; x1: number; z0: number; z1: number; h: number; }
...
export class Minimap {
  ...
  draw(px: number, pz: number, yaw: number): void {
    ...
    const rects: MRect[] = this.objs.filter((o) => SOLID.has(o.t)).map((o) => {
      ...
      return { x0: o.x - fp.hw, x1: o.x + fp.hw, z0: o.z - fp.hd, z1: o.z + fp.hd, h: o.h };
    });
    ...
    for (const r of rects) {
      ...
      if (!rects.some((o) => o !== r && Math.abs(o.z1 - r.z0) < E && ovX(o, r))) { ctx.moveTo(L, T); ctx.lineTo(R, T); }
      if (!rects.some((o) => o !== r && Math.abs(o.z0 - r.z1) < E && ovX(o, r))) { ctx.moveTo(L, B); ctx.lineTo(R, B); }
      if (!rects.some((o) => o !== r && Math.abs(o.x1 - r.x0) < E && ovZ(o, r))) { ctx.moveTo(L, T); ctx.lineTo(L, B); }
      if (!rects.some((o) => o !== r && Math.abs(o.x0 - r.x1) < E && ovZ(o, r))) { ctx.moveTo(R, T); ctx.lineTo(R, B); }
    }
    ctx.stroke();
