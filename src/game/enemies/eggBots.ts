...
const TURN_SMOOTH = 7;      // 转身平滑速度(越大转得越快)
...
export class EggBots {
  ...
  update(dt: number, playerPos: THREE.Vector3): void {
    for (const b of this.bots) {
      ...
      let mvx = 0, mvz = 0;               // 这帧的移动方向(用来朝向)
      if (wp) {
        let dx = wp.x - p.x, dz = wp.z - p.z;
        const d = Math.hypot(dx, dz);
        if (d < 0.55) { b.pathI++; }       // 到这个拐点了 → 去下一个
        else { dx /= d; dz /= d; p.x += dx * SPEED * dt; p.z += dz * SPEED * dt; mvx = dx; mvz = dz; }
      }
      pushOut(p, this.solids, 0.5);

      // 朝向：玩家走近(注意范围内)就转头看玩家，否则看移动方向；平滑转身(不再生硬瞬转)
      const dpx = playerPos.x - p.x, dpz = playerPos.z - p.z;
      const distP = Math.hypot(dpx, dpz);
      let fx = mvx, fz = mvz;
      if (distP > 0.6 && distP < DETECT) { fx = dpx; fz = dpz; } // 看向玩家
      if (fx !== 0 || fz !== 0) {
        const targetYaw = Math.atan2(fx, fz);
        let dy = targetYaw - b.group.rotation.y;
        dy = Math.atan2(Math.sin(dy), Math.cos(dy)); // 归一化到 [-π,π] 走最近的方向转
